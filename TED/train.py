...
def make_env(cfg, test=False,panda_gym=True):
    # per dreamer: https://github.com/danijar/dreamer/blob/02f0210f5991c7710826ca7881f19c64a012290c/wrappers.py#L26
    camera_id = 2 if cfg.domain_name == 'quadruped' else 0

    if panda_gym:
        env_1 = gym.make("PandaReachDense-v3", render_mode="rgb_array")
        env = pandaWrapper.PandaObsWrapper(env_1, cfg.seed, frame_skip=1, k=cfg.frame_stack, img_size=84, greyscale=True)
    if not panda_gym:
        if test:
            env = dmc2gym.make(domain_name=cfg.domain_name,
                               ...
                               background_dataset_path=None if cfg.test_background_dataset_path == "None" else os.path.join(
                                   os.getcwd(), cfg.test_background_dataset_path),
            ...
            env = dmc2gym.make(domain_name=cfg.domain_name,
                               ...
                               dynamic=cfg.dynamic,
                               background_dataset_path=None if cfg.background_dataset_path == "None" else os.path.join(
                                   os.getcwd(), cfg.background_dataset_path),
